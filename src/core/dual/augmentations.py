...
def compute_cutmix(device, h, w, imgs, labels, criterion, model, ema_model, image_u, threshold):
    with torch.no_grad():
    ...
    image_aug, label_aug = cut_mixer(device, image_u, pred_label.clone())

    # image_aug, label_aug, pred_logit = \
    #     batch_transform(device, image_aug, label_aug, pred_logit,
    ...
    pred_large = F.interpolate(outputs, size=labels.shape[2:], mode='bilinear', align_corners=False)
    sup_loss = criterion(pred_large, labels.clone())
...
def batch_transform(device, data, label, logits, crop_size, scale_size, apply_augmentation):
    ...
    data_trans, label_trans, logits_trans = \
        torch.cat(data_list).to(device=device), torch.cat(label_list).to(device=device), torch.cat(logits_list).to(device=device)
...
def cut_mixer(device, data, target):
    target = target.unsqueeze(dim=1)
    mix_data = data.clone()
    mix_target = target.clone()
    u_rand_index = torch.randperm(data.size()[0])[:data.size()[0]].to(device=device)
...
def get_bin_mask(device, b, argmax_occluder):
    for image_i in range(b):
        classes = torch.unique(argmax_occluder[image_i])

        classes = classes[classes != 255]
        nclasses = classes.shape[0]
        classes = (classes[torch.Tensor(np.random.choice(nclasses, int((nclasses - nclasses % 2) / 2), replace=False)).long()]).to(device=device)
        if image_i == 0:
            binary_mask = generate_class_mask(argmax_occluder[image_i], classes).unsqueeze(0).to(device=device)
        else:
            binary_mask = torch.cat((binary_mask, generate_class_mask(argmax_occluder[image_i], classes).unsqueeze(0).to(device=device)))
    return binary_mask


def compute_classmix(device, b, h, w, criterion, cm_loss_fn, model, ema_model, imgs, labels, unsup_imgs, image_u_strong, threshold):
    # Unlabeled Process
    with torch.no_grad():
    ...
    binary_mask = get_bin_mask(device, b, argmax_occluder)
    #binary_mask = binary_mask.squeeze(dim=1)
    binary_mask = binary_mask.unsqueeze(dim=1)
    if b == 2:
        shuffle_index = torch.tensor([1, 0])
    else:
        shuffle_index = torch.randperm(b).to(device=device)
    ...
    pixel_weight = unlabeled_weight * torch.ones(max_prob_occluder.shape).to(device=device)
...
def compute_ic(model, ema_model, image_u, image_u_strong, criterion_u, label_u, h, w, threshold):
    ...
    loss_dc = loss_dc * ((max_probs >= threshold))
