...
def train_net(net_1,
    ...
    sourceDir = f'{os.getcwd()}/LIVECell_dataset/train_data/{source}'
    targetDir = f'{os.getcwd()}/LIVECell_dataset/train_data/{target}'
    # load train images
    ...
    for epoch in range(epochs):
        ...
        if with_source:
            for i, (bs, bt) in enumerate(zip(batch(train_s, batch_size, source), batch(train_t, batch_size, source))):
                ...
                count += 1
                if i != 0:
                    print('epoch : {}, iter : {}, loss_1 : {}'.format(epoch + 1, i, epoch_loss_1 / i))
        ...
        ### targetのみの場合
            ...
            for i, bt in enumerate(batch(train_t, batch_size, source)):
                ...
                with torch.no_grad():
                    ...
                    # pseudo_prob = ( lg1_prob + lg2_prob ) / 2
                    pseudo_bin = (lg1_prob > 0.5).float()
                    pseudo_bin_flat = pseudo_bin.view(-1).detach()
        ...
        already = False
        if ((epochs - epoch) == 50) or ((epochs - epoch) < 3):
            ...
            torch.save({
                ...
                'opt_net1': best_opt1,
                # 'opt_net2' : best_opt2,
            }, '{}CP_{}_min_segloss1_e{}'.format(dir_checkpoint, step, epoch + 1))

            already = True
            with open(path_w, mode='a') as f:
                f.write('seg loss 1 is update \n')

        my_dict = {'tr_s_loss_list_1': tr_s_loss_list_1,
    ...
    draw_graph(dir_graphs, 'train_loss', epochs, red_list=tr_s_loss_list_1, red_label='loss1')

    draw_graph(dir_graphs, 'train_source_loss', epochs, red_list=source_seg_list_1, red_label='source loss1')

    draw_graph(dir_graphs, 'train_target_loss', epochs, red_list=tgt_seg_list_1, red_label='target loss1')
...
if __name__ == '__main__':
    ...
    stepkey = '' if args.step == 0 else f'_{(args.step - 1) * 100}_{args.step * 100}'
    ### difine forget_rate
