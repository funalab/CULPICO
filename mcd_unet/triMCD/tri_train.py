...
def train_net(net_g,
    ...
    for epoch in range(epochs):
        ...
        #( minimum s_loss or d_loss 更新時checkpoint saved)org

        ###process0時 source segloss 最小と target pseudo loss最小モデルを保存###
...
def get_args():
    ...
    parser.add_argument('-cop', '--co_process0', type=float, nargs='?', default=1.0,
                        help='the coefficient in process0?', dest='co_P')
    ...
    if args.contrain != None:
        ...
        for state in opt_s2.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(device=device)
        for state in opt_s0.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(device=device)
    else:
