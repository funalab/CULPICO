...
def create_ensemble_pseudo( p1, p2, device='cpu' ):
    p_mean = ( p1 + p2 ) / 2

    pseudo_lab = torch.where( p_mean > 0.5, torch.tensor(1, dtype=p_mean.dtype, device=torch.device(device)), torch.tensor(0, dtype=p_mean.dtype, device=torch.device(device)) )
